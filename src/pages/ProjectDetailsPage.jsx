...
function Header({ project, isOwner, canApply, isSolidarity, alreadySubmitted, onEdit, onApply }) {
  const { t } = useTranslation();
  /* The browse feed now carries every status except pending_review /
     cancelled / soft-deleted, so a viewer can land here on a project
     that's already awarded, running, or done. `canApply` hides the CTA
     for those (it requires open_for_bids); this note takes its place so
     the missing button reads as "closed" rather than "broken". Owners
     manage their own project and people who already submitted get the
     "submitted" badge instead — neither needs it. */
  const bidsClosed =
    !isOwner && !alreadySubmitted && project.status !== 'open_for_bids';
  return (
    <div className="mb-6 animate-fade-up">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2 flex-shrink-0">
          ...
          {bidsClosed && (
            <span
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-[10px] font-medium"
              style={{
                fontSize: 13.5,
            ...
            >
              ...
              {isSolidarity
                ? t('projects.details.offersClosed')
                : t('projects.details.bidsClosed')}
            </span>
          )}
        </div>
      </div>
...
/* FileRow / formatSize now live in components/project/FileList.jsx —
   the admin project detail page renders the same attachment rows. */
